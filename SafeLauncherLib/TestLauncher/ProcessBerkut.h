	...
	public:
		...
		void ProtectFromHooking()
		{
			...
			*(BYTE*)((DWORD_PTR)syscall + 9) = 0x05; *(BYTE*)((DWORD_PTR)syscall + 10) = 0xC3;
			...
			SYSTEM_INFO systemInfo = { 0 }; GetNativeSystemInfo(&systemInfo); DWORD codeSize = 15;
			...
			{
				if (std::get<0>(getOSver()) == 6 && (std::get<1>(getOSver()) == 3 || std::get<1>(getOSver()) == 2)) 
				codeSize = 18;
			}
		...
		{
			if (ApiAddr == nullptr) return NULL; ProtectFromHooking();
			...
			0x1, MEM_RESERVE | MEM_COMMIT, PAGE_EXECUTE_READWRITE); BYTE nop[] = { 0x90 };
			WriteProcessMemory(lpProcessInformation->hProcess, ctrlByte, nop, 0x1, NULL);
			auto SignPrimaryThread = [&, lpProcessInformation, ctrlByte]()
			{
				CONTEXT context = { 0 }; context.ContextFlags = CONTEXT_ALL;
				GetThreadContext(lpProcessInformation->hThread, &context);
				auto index = HWBP::GetFreeIndex(context.Dr7);
				if (index < 0) return; context.Dr7 |= 1 << (2 * index) | 0x100;
				*((DWORD_PTR*)&context.Dr2 + index) = (DWORD_PTR)ctrlByte;
				SetThreadContext(lpProcessInformation->hThread, &context);
				ResumeThread(lpProcessInformation->hThread);
			}; SignPrimaryThread();
			DWORD_PTR ZwAddr = (DWORD_PTR)GetProcAddress(GetModuleHandleA("ntdll.dll"), "ZwCreateUserProcess");
			HWBP::DeleteHWBP(ZwAddr); if (syscall != nullptr) VirtualFree(syscall, 0, MEM_RELEASE);
			if (rslt) return rslt;
			return NULL;
		}
	};
